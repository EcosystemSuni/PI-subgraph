import {
    SUSHI_PIDDAI_PAIR, SUSHI_XSUSHI_ETH_PAIR, SUSHI_USDC_ETH_PAIR
} from './Constants'
...
export function getPIDUSDRate(): BigDecimal {
    let pair = UniswapV2Pair.bind(Address.fromString(SUSHI_PIDDAI_PAIR))

    let reserves = pair.getReserves()
    let reserve0 = reserves.value0.toBigDecimal()
    let reserve1 = reserves.value1.toBigDecimal()

    let PIDRate = reserve1.div(reserve0).div(BIG_DECIMAL_1E9)
    log.debug("PID rate {}", [PIDRate.toString()])
...
//(slp_treasury/slp_supply)*(2*sqrt(lp_dai * lp_PID))
...
export function getPairUSD(lp_amount: BigInt, pair_adress: string): BigDecimal{
    ...
    let PID_value = toDecimal(lp_token_0, 9).times(getPIDUSDRate())
    let total_lp_usd = PID_value.plus(toDecimal(lp_token_1, 18))
