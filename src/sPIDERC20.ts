...
import { PID_ERC20_CONTRACT, STAKING_CONTRACT_V2 } from './utils/Constants'
import { toDecimal } from './utils/Decimals'
import { getPIDUSDRate } from './utils/Price';
...
export function rebaseFunction(call: RebaseCall): void {
    ...
    if (rebase == null && call.inputs.profit_.gt(BigInt.fromI32(0))) {
        ...
        rebase.stakedPIDs = toDecimal(PID_contract.balanceOf(Address.fromString(STAKING_CONTRACT_V2)), 9)
        rebase.contract = STAKING_CONTRACT_V2
        rebase.percentage = rebase.amount.div(rebase.stakedPIDs)
        rebase.transaction = transaction.id
        rebase.timestamp = transaction.timestamp
        rebase.value = rebase.amount.times(getPIDUSDRate())
        rebase.save()
